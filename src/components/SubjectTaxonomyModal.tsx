...
export const SubjectTaxonomyModal = ({ campusLabel, taxonomy, onSave, onClose }: Props) => {
  ...
  const addFaculties = () => {
    const values = splitCommaValues(facultyInput);
    if (values.length === 0) return;
    setFaculties(prev => normalizeList([...prev, ...values]));
    setFacultyInput('');
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer' }} aria-label="close">
            <X />
          </button>
        </header>
        ...
        <div style={{ padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          <section style={{ border: '1px solid #e5e7eb', borderRadius: '8px', padding: '16px' }}>
            <h4 style={{ margin: '0 0 12px 0' }}>開講学部</h4>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {faculties.length === 0 && <span style={{ color: '#999', fontSize: '0.85rem' }}>登録がありません</span>}
              {faculties.map(item => (
                <span key={item} style={chipStyle('faculty')}>
                  {item}
                  <button
                    onClick={() => setFaculties(prev => prev.filter(v => v !== item))}
          ...
          <section style={{ border: '1px solid #e5e7eb', borderRadius: '8px', padding: '16px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {departments.length === 0 && <span style={{ color: '#999', fontSize: '0.85rem' }}>登録がありません</span>}
              {departments.map(item => (
                <span key={item} style={chipStyle('department')}>
                  {item}
                  <button
                    onClick={() => setDepartments(prev => prev.filter(v => v !== item))}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#c62828' }}
                    aria-label={`remove-${item}`}
                  >
                    <Trash2 size={14} />
        ...
        >
          <button onClick={onClose} style={{ padding: '8px 14px', border: '1px solid #ccc', borderRadius: '6px', background: '#fff' }}>
            キャンセル
          </button>
          <button
            onClick={handleSave}
