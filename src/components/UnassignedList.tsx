...
export const UnassignedList = ({ subjects, allocations, onReorder, onDragStart, onDragEnd, onEdit, onRemoveAllocation }: Props) => {
    ...
    // 存在する時限のパターン（1, 1-2 など）を抽出
    const periodPatterns = Array.from(new Set(subjects.map(s =>
            ...
            <div style={{ borderBottom: '1px solid #ddd', paddingBottom: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    <h3 style={{ margin: 0, fontSize: '1.0rem', color: '#333' }}>
                        未割り当て ({filteredSubjects.length})
                    </h3>
                    {(selectedTerms.size > 0 || selectedDays.size > 0 || selectedPeriods.size > 0) && (
                        <button
                            onClick={() => {
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <div style={{ display: 'flex', gap: '4px' }}>
                        <FilterDropdown
                            ...
                            getLabel={(val: Term) => val === 'spring' ? '春' : val === 'autumn' ? '秋' : '通年'}
                        />
                        <FilterDropdown
                            label="すべての曜日"
                    ...
                    <FilterDropdown
                        label="すべての時限"
                        options={periodPatterns.map(p => ({ value: p, label: p.includes('-') ? `${p.split('-')[0]}〜${p.split('-')[1]}講時` : `${p}講時` }))}
                        selected={selectedPeriods}
                        onToggle={(val: string) => toggleFilter(selectedPeriods, setSelectedPeriods, val)}
                        isOpen={isPeriodOpen}
                        setIsOpen={setIsPeriodOpen}
                        getLabel={(val: string) => val.includes('-') ? `${val.split('-')[0]}-${val.split('-')[1]}` : `${val}講時`}
                    />
                </div>
            </div>
                ...
                >
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '0.7rem', color: '#888' }}>
                            {subject.term === 'spring' ? '春' : subject.term === 'autumn' ? '秋' : '通年'} {DAY_LABELS[subject.day]} {subject.period}講時
                            {subject.endPeriod && subject.endPeriod > subject.period && ` 〜 ${subject.endPeriod}講時`}
                        </div>
